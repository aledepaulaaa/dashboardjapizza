...
const useProductSubmit = (id) => {
  ...
  console.log("Título do Produto: ", resData.title);
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      if (!imageUrl) return notifyError("Imagem requerida!");

      if (data.originalPrice < data.price) {
        setIsSubmitting(false);
        return notifyError(
          "O preço de venda deve ser menor ou igual ao preço do produto!"
        );
      }
      if (!defaultCategory[0]) {
        setIsSubmitting(false);
        return notifyError("Categoria padrão é obrigatória!");
      }
